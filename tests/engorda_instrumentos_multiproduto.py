#!/usr/bin/env python3
"""
engorda_instrumentos.py — motor multi-produto de clonagem por entidade.
...
# fontes-subtipo clonáveis. As fontes são RAW; a poda restringe explicitamente
# apenas o pai CONDICAO_IF às linhas ativas, pois essa é uma guarda de integridade
# do carregamento e não um filtro de negócio do fecho clonado.
# ---------------------------------------------------------------------------
...
REGRAS_PRODUTO: Dict[str, Dict[str, Any]] = {
    "cdb_simplificado": {
        ...
        "integridade": {
            ...
            "faltantes_seletivos": (("OPERACAO", "NUM_ID_CTX_MSG_P2"),),
        },
        "chaves_negocio": {
...
REGRAS_PRODUTO["cdb"]["prefixo_saida"] = f"{DEFAULT_CLONE_PREFIX}/cdb_completo"

REGRAS_PRODUTO["rdb"] = copy.deepcopy(REGRAS_PRODUTO["cdb_simplificado"])
REGRAS_PRODUTO["rdb"]["arquivo_sql"] = "rdb.sql"
REGRAS_PRODUTO["rdb"]["prefixo_saida"] = f"{DEFAULT_CLONE_PREFIX}/rdb_completo"
...
def _build_product_profile(name: str, raw: Mapping[str, Any]) -> ProductProfile:
    ...
    if subtype_raw is not None:
        ...
        for raw_type, raw_table in subtype_tables.items():
            if not isinstance(raw_type, str) or not raw_type.strip():
                raise ValueError(f"{name}.integridade.subtipo contém tipo inválido")
    ...
    for raw_table, raw_columns in nullify_raw.items():
        ...
        for raw_column in _rule_sequence(raw_columns, f"{name}.{table}.nulificar_colunas"):
            column = _normalize_rule_identifier(raw_column, f"{name}.{table}.nulificar_colunas")
    ...
    for item in _rule_sequence(
        integrity_raw["faltantes_seletivos"], f"{name}.integridade.faltantes_seletivos"
    ):
        pair = _rule_sequence(item, f"{name}.faltantes_seletivos")
        if len(pair) != 2:
            raise ValueError(f"{name}.faltantes_seletivos precisa conter pares (TABELA, COLUNA)")
        normalized_selective.add(
            (
                _normalize_rule_identifier(
                    pair[0], f"{name}.faltantes_seletivos.tabela", table=True
                ),
                _normalize_rule_identifier(pair[1], f"{name}.faltantes_seletivos.coluna"),
            )
        )
    ...
    return ProductProfile(
        ...
        static_tables=tuple(
            _normalize_rule_identifier(table, f"{name}.tabelas_static", table=True)
            for table in _rule_sequence(rules["tabelas_static"], f"{name}.tabelas_static")
        ),
...
PRODUCT_PROFILES: Mapping[str, ProductProfile] = MappingProxyType(
    {name: _build_product_profile(name, rules) for name, rules in REGRAS_PRODUTO.items()}
)
CDB_SIMPLIFICADO_PROFILE = PRODUCT_PROFILES["cdb_simplificado"]


def _validate_product_profile(profile: ProductProfile) -> None:
    if not isinstance(profile.name, str) or not re.fullmatch(r"[a-z][a-z0-9_]*", profile.name):
...
def get_product_profile(name: str) -> ProductProfile:
    ...
    if profile.name != normalized:
        raise ValueError(
            f"registro de produto inconsistente: chave {normalized!r}, perfil {profile.name!r}"
        )
...
def _validate_product_registry() -> None:
    ...
    for key, profile in PRODUCT_PROFILES.items():
        if key != profile.name:
            raise ValueError(
                f"registro de produto inconsistente: chave {key!r}, perfil {profile.name!r}"
            )
        _validate_product_profile(profile)
        prefix = _normalize_clone_prefix(profile.default_clone_prefix).lower()
...
def _read_num_if_query_text(
    spark: SparkSession, query_path: Optional[str], default_filename: Optional[str] = None
) -> Tuple[str, str]:
    ...
    while remaining:
        ...
        if not ready:
            if not warned:
                ...
                logger.warning(
                    "Ciclo de FK entre tabelas clonáveis; quebrando "
...
def aplica_regras_engorda(
    ...
    *,
    ...
    if tem_venc:
        venc_expr = F.expr(
            f"date_add(DATE '{engorda_dt.isoformat()}', CAST({ENGORDA_PRAZO_TMP_COL} AS INT))"
        )
        df = df.withColumn(
...
def _dominio_num_if_produto(
    ...
    """
    _validate_product_profile(profile)
    sql_text, resolved_path = _read_num_if_query_text(spark, query_path, profile.query_filename)
...
def _subtipos_clonaveis(spec: dict, policy: SubtypePolicy) -> List[Tuple[str, str]]:
    """Pares (tipo, tabela-subtipo) que o clone realmente produz."""
    return [
        (str(tipo), tabela)
...
def _carrega_faltantes(
    ...
    if df is None:
        return None
    return df.select(
        F.col("TABELA"), F.col("COLUNA"), _norm_key_col(F.col("VALOR")).alias("VALOR")
    ).dropDuplicates()
...
def _num_if_excluidos_por_faltantes(
    ...
    for tab, col in pares:
        ...
        miss = (
            ...
            .select(F.col("VALOR").alias("__v"))
            .dropDuplicates()
        )
        hit = (
            src.select(F.col(COL_NUM_IF).alias(COL_NUM_IF), _norm_key_col(F.col(col)).alias("__v"))
...
def aplica_nulificacao_faltantes(
    ...
    for tab, coluna in sorted(selective_keys):
        ...
        if coluna not in out.columns:
            raise ValueError(f"{tab}.{coluna}: coluna allowlisted não existe no schema dos clones.")
        colisao = [
            c
        ...
        joined = out.withColumn(FALTANTES_SELECTIVE_KEY_COL, _norm_key_col(F.col(coluna))).join(
...
def seleciona_instrumentos(
    ...
    fonte = (
        _dominio_num_if_produto(spark, config, profile, query_num_if_path)
        .select(COL_NUM_IF)
        .dropDuplicates()
    )
    logger.info("Produto %s: domínio de NUM_IF vindo integralmente da query.", profile.name)
...
def calcula_lotes(
    ...
    for passada in range(1, max_passadas + 1):
        ...
        for t in ordem:
            ...
            for fk in fks_uteis:
                chaves_pai = (
                    lotes[fk.parent_table]
                    .select(*[F.col(pc).alias(cc) for cc, pc in zip(fk.columns, fk.parent_columns)])
                    .dropDuplicates()
                )
                partes.append(
                    src.join(F.broadcast(chaves_pai), on=list(fk.columns), how="left_semi")
                )
            lote_t = partes[0]
...
def clona_tabela(
    spark, plano: PlanoTabela, lote: DataFrame, fator_k: int, mapeamentos: Dict[str, DataFrame]
...
def _generate_meu_numeros(operacoes: DataFrame, prefix: str, engorda_date: date) -> DataFrame:
...
def _validate_disabled_operation_output(
    ...
    """Impede que operacoes clonadas mantenham chaves de negócio da origem."""
    ...
    if source_rows:
        raise ValueError(
            ...
            "REGRAS_PRODUTO ou exclua OPERACAO da clonagem em tabelas_static."
        )
...
def valida_tabela(
    ...
    if total != esperado:
        erros.append(
            f"contagem: {total} clone(s), esperado {esperado} (lote {n_lote} × K {fator_k})"
        )
...
def _stage_and_publish(
    spark: SparkSession, final_path: str, prepare: Callable[[str], None]
) -> None:
...
def executa_clonagem(
    ...
    *,
    ...
    if product_profile.date_strategy == "standard":
        logger.info(
            "Data de engorda do run: %s (prazo de %s: %s)",
    ...
    if faltantes is not None:
        logger.info(
            "Filtro de faltantes (itens 3/4) ativo: %d chave(s) de "
            "referência inexistentes no destino.",
            faltantes.count(),
        )
    ...
    for t in ordem:
        ...
        if product_profile.date_strategy == "standard":
            clones, cols_data = aplica_regras_engorda(
                clones, t, engorda_ts=engorda_ts, prazo_vencimento_dias=prazo_vencimento_dias
            )
        else:
            cols_data = []
        ...
        logger.info(
            "[%s] lote=%d clones=%d remapeadas=%s datas=%s anuladas=%s %s",
            t,
            n_lote,
            n_lote * fator_k,
            cols_remap,
    ...
    if operation_policy is not None and operation_policy.table not in resultados:
        raise ValueError(f"política de chaves de negócio exige a tabela {operation_policy.table}")
    ...
    def _prepare_outputs(output_base: Optional[str], is_dry_run: bool) -> None:
        ...
        if operation_policy is not None:
            ...
            if operation_policy.generate_meu_numero:
                operacoes = _generate_meu_numeros(operacoes, meu_numero_prefix, engorda_ts.date())
        ...
        if not is_dry_run and operation_policy is not None and operation_policy.generate_meu_numero:
    ...
    if dry_run:
        ...
        _stage_and_publish(
            spark, save_base, lambda staging_base: _prepare_outputs(staging_base, False)
        )
        logger.info("Staging validado e publicado em %s.", save_base)

    logger.info("=" * 78)
    logger.info(
        "RESUMO DA CLONAGEM (%.1fs) — %d instrumento(s) × K=%d, data de engorda %s, %s",
    ...
    for t in ordem:
        s = stats.get(t, {})
        logger.info(
            "  %-32s lote=%-8s clones=%-8s remap=%s datas=%s anuladas=%s",
            t,
            s.get("lote", "-"),
            s.get("clones", "-"),
...
def parse_arguments(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Motor multi-produto de clonagem por entidade (clone-and-remap), "
    ...
    parser.add_argument(
        "--fator-k", type=positive_int, default=1, help="Clones por instrumento (default 1)."
    )
    parser.add_argument(
        "--meu-numero-prefix",
    ...
    parser.add_argument(
        "--tratar-como-static", default="", help="Tabelas a excluir da clonagem (vírgula)."
    )
    parser.add_argument(
        "--max-passadas",
        type=positive_int,
    ...
    parser.add_argument(
        "--specs", default=None, help="Override de DATAGEN_SPECS_URI (specs.json único)."
    )
