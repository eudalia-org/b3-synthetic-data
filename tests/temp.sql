...
-- NO DESTINO. Esperado: ~583 e ~1107, iguais à origem.
SELECT (SELECT COUNT(DISTINCT S.NUM_ID_LOTE)
          FROM CREDITO_SCR S INNER JOIN LOTE L ON L.NUM_ID_LOTE = S.NUM_ID_LOTE
         WHERE S.DAT_EXCLUSAO IS NULL AND L.NUM_ID_TIPO_LOTE = 1) AS LOTES_T1_COM_SCR,
       (SELECT COUNT(DISTINCT D.NUM_ID_LOTE)
          FROM CREDITO_DC D INNER JOIN LOTE L ON L.NUM_ID_LOTE = D.NUM_ID_LOTE
         WHERE D.DAT_EXCLUSAO IS NULL AND L.NUM_ID_TIPO_LOTE = 2) AS LOTES_T2_COM_DC
FROM DUAL;
