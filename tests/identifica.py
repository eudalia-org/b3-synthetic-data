...
# ---------------------------------------------------------------------------
def descida_instrumentada(
    spark, config, comp_specs: dict, limit: Optional[int]
) -> Dict[str, DataFrame]:
    order = et.topo_order_tables(comp_specs)
    ...
    for table in order:
        df = et._aplica_filtro_tipo_if(et.read_parquet(spark, et.raw_path(config, table)), table)
        n_raiz = df.count()
        eh_raiz = table in et.TABELAS_RAIZ_FILTRO
        rotulo_raiz = f"  [RAIZ: filtro NUM_TIPO_IF={et.FILTRO_TIPO_IF_VALUE}]" if eh_raiz else ""
        print(f"\n### {table}{rotulo_raiz}")
        ...
        for fk in et._fk_list(comp_specs[table]):
            ...
            if parent not in sampled or not cols or len(cols) != len(pcols):
                motivo = (
                    "PAI AINDA NAO AMOSTRADO (ciclo!)"
                    if parent in comp_specs
                    else "pai fora do componente"
                )
            ...
            cond = reduce(
                lambda a, b: a & b, [df[cols[i]] == keys[f"__k{i}"] for i in range(len(cols))]
            )
            ...
            df = joined.where(F.col("__k0").isNotNull() | all_fk_null).drop(
                *[f"__k{i}" for i in range(len(pcols))]
            )
...
def fecho_instrumentado(
    ...
    for passe in range(1, max_passes + 1):
        ...
        for child in reversed(order):
            ...
            for fk in et._fk_list(comp_specs[child]):
                ...
                cols = list(fk.get("columns") or [])
                pcols = list(fk.get("parent_columns") or [])
                if not cols or len(cols) != len(pcols) or parent not in sampled:
                    continue
                ...
                faltantes = ref_keys.join(
                    alvo_df.select(*pcols).distinct(), on=pcols, how="left_anti"
                ...
                extra = (
                    et.read_parquet(spark, et.raw_path(config, alvo))
                    .join(faltantes_side, on=pcols, how="left_semi")
                    .localCheckpoint(eager=True)
                )
                n_puxadas = extra.count()
                print(
                    f"    {child}.{','.join(cols)} -> {alvo}"
                ...
                if n_puxadas > 0:
                    sampled[alvo] = sampled[alvo].unionByName(extra).localCheckpoint(eager=True)
    ...
    if total_por_passe and total_por_passe[-1] > 0:
        print(
            f">>> ATENCAO: nao convergiu em {max_passes} passes — investigar antes de qualquer fix."
        )
    return sampled
...
# ---------------------------------------------------------------------------
def classifica_orfaos(spark, config, comp_specs: dict, sampled: Dict[str, DataFrame]) -> None:
    print(SEP)
    ...
    for child, cfg in comp_specs.items():
        ...
        for fk in et._fk_list(cfg):
            ...
            parent_keys = base_pai.select(
                *[F.col(p).alias(c) for c, p in zip(cols, pcols)]
            ).distinct()
            orfas = (
                child_df.select(*cols)
                .dropna()
                .distinct()
            ...
            algum = True
            full_keys = (
                et.read_parquet(spark, et.raw_path(config, parent))
            ...
            destino = "ANULADA (nao-PK)" if nullable_no_spec else "DROPADA (FK dentro da PK)"
            print(f"\n### {child}.{','.join(cols)} -> {parent}{' (self)' if eh_self else ''}")
...
# ---------------------------------------------------------------------------
def dominio_full_por_chave(spark, config, comp_specs: dict, zeradas: List[str]) -> None:
    print(SEP)
    ...
    for table in order:
        ...
        for fk in et._fk_list(comp_specs[table]):
            ...
            pcols = tuple(fk.get("parent_columns") or [])
            key = (parent, pcols)
            if parent == table or key not in domain_keys or not cols or len(cols) != len(pcols):
                continue
        ...
        if table in zeradas:
            marca = (
                "  <== Z1: ARTEFATO DO --limit (dominio completo tem linhas)"
                if n_dom > 0
                else "  <== Z2: dominio realmente vazio (zero e correto)"
            )
        ...
        for pcols in parent_refs.get(table, set()):
            domain_keys[(table, pcols)] = (
                df.select(*[F.col(pc).alias(f"__k{i}") for i, pc in enumerate(pcols)])
                .dropna()
                .distinct()
                .localCheckpoint(eager=True)
            )
...
def main() -> None:
    ...
    ap.add_argument(
        "--tabelas",
        default=None,
        help="Foco: roda so o(s) componente(s) que contem estas tabelas (separadas por virgula).",
    )
    ap.add_argument(
        "--sem-dominio-full",
        action="store_true",
    ...
    try:
        ...
        foco = set(t.strip().upper() for t in args.tabelas.split(",")) if args.tabelas else None
...
# Não será gravado, mas o env exige.
os.environ["DATAGEN_SYNTHETIC_BASE_URI"] = "oci://.../<prefixo_synthetic>"
...
spark = (
    SparkSession.builder.appName("DiagnosticaSampling")
    .master("local[*]")
...
# Notebook example using the diagnostic functions defined above.
...
violadas = relatorio_ciclos(comp_specs)


# [2] descida com contadores por aresta — identifica a FK que zerou cada tabela
sampled = descida_instrumentada(spark, config, comp_specs, limit=10000)
zeradas = [t for t, df in sampled.items() if df.count() == 0]
