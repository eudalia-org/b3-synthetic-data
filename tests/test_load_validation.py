...
class TestDryRunArg:
    REQUIRED_ARGS = [
        "load_tables",
        "--validation-report",
        "oci://bucket@ns/run/validation.json",
        "--product",
        "cdb_simplificado",
        "--validation-product",
        "cdb_simplificado",
        "--manifest-uri",
        "oci://bucket@ns/run/load-manifest.json",
        "--pipeline-manifest-uri",
        "oci://bucket@ns/run/pipeline-manifest.json",
        "--expected-target-schema",
        "CETIP",
    ]

    def test_dry_run_flag_parses(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", [*self.REQUIRED_ARGS, "--dry-run"])
        args = L.parse_arguments()
        assert args.dry_run is True

    def test_dry_run_defaults_false(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", self.REQUIRED_ARGS.copy())
        args = L.parse_arguments()
