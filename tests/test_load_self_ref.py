...
def spark():
    ...
    s = (
        SparkSession.builder.appName("load-selfref-test")
        .master("local[2]")
...
class TestNullSelfRefColumns:
    ...
    def test_nulls_listed_columns_keeps_others(self, spark):
        ...
        rows = {
            r["DESC"]: (r["NUM_IF"], r["NUM_IF_ORIGEM"], r["NUM_IF_PERTENCE"])
    ...
    def test_missing_column_skipped(self, spark):
        ...
        df = spark.createDataFrame([(1,)], T.StructType([T.StructField("NUM_IF", T.LongType())]))
    ...
    def test_constant_contents(self):
        assert L.NULL_ON_INSERT == {"INSTRUMENTO_FINANCEIRO": ["NUM_IF_ORIGEM", "NUM_IF_PERTENCE"]}
