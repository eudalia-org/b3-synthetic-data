...
# Notebook input: supplied by the caller, not initialized by this snippet.
...
def orphan_count_safe(child_df, parent_df, cols, pcols):
    parent_keys = (
        parent_df.select(*[F.col(p).alias(c) for c, p in zip(cols, pcols)]).dropna().distinct()
    )
...
for child, cols, parent, pcols in CHECKS:
    ...
    print(
        f"{child}.{','.join(cols)} -> {parent}.{','.join(pcols)}: "
